...
from .confighandler import get_folder
...
def get_data(tid, lid, aid, dfrom, dto, rent):
    ...
    apartments = pd.DataFrame(get_spreadsheet('apartments').get_all_records())
    tennants = pd.DataFrame(get_spreadsheet('tennants').get_all_records())
    landlords = pd.DataFrame(get_spreadsheet('landlords').get_all_records())
...
def prepare_agreement(tennant, landlord, apartment, dfrom, dto, rent):
    ...
    path = get_folder('agreements')+details['[8]'].replace(" ", "_")
